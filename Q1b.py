...
# As a mitigation, I will use the POX controller to run the topology
# Run pox controller first
# cd pox
# ./pox.py openflow.discovery openflow.spanning_tree forwarding.l2_learning stp-config
# For this, Run the command sudo python3 t1.py  after running the POX controller
...
topo = A3Q1b() # I create the topology that is given in Question 1
net = Mininet(topo=topo, controller=None, link=TCLink) # I create the network with the topology that I created above
net.addController('pox', controller=RemoteController, ip='127.0.0.1', port=6633) # I add the POX controller to the network. 
